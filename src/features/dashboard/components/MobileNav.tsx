...
import { CalendarDays, House, LogOut, Plus, Users } from "lucide-react";
...
const MobileNav = () => {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-50 flex h-20 items-center justify-around border-t bg-white/95 px-4 shadow-[0_-8px_25px_rgba(15,23,42,0.08)] backdrop-blur md:hidden">
      {navItems.slice(0, 2).map(({ icon: Icon, title, route, function: action }) => (
        <NavLink
          to={route}
          key={title}
          onClick={action}
      ...
      {navItems.slice(2).map(
        ({ icon: Icon, title, route, function: action }) => (
          <NavLink
            to={route}
            key={title}
            onClick={action}
